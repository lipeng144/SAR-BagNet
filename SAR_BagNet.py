...
import numpy as np
#stride=1 RF=19 p=0
model_urls = {'Bagnet18_1':'file_name'}
...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        #     # logits2 = logits2.reshape((82, 82))
        # fig = plt.figure()
        # ax = fig.add_subplot(111)
        # ax.imshow(logits2, cmap=plt.cm.jet)
        # plt.show()
        if self.avg_pool:
...
def BagNet18(pretrained=False):
    ...
    if pretrained:
        my_dict = model_zoo.load_url(model_urls['Bagnet18_1'], model_dir=model_dir)
